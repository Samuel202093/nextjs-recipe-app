...
const Item:React.FC<RecipeProps> = ({recipe}) => {
  ...
  return (
    <>
         <div className="recipeContainer flex flex-col gap-2 w-[30%]  min-h-[15vh] border-2 rounded-md p-4" key={recipe._id}>
                    <div className="border-[1px]y border-blacky w-[60%] mx-auto min-h-[20vh] rounded-md">
                    {imageUrl ? <img src={imageUrl} alt="recipe-img" className="w-full object-contain border-2y border-red-600y h-fit rounded-md"/> : <img src={recipeImg.src} alt="recipe-img" className="w-full object-contain border-2y border-red-600y h-fit rounded-md"/>}
                    </div>

                    <div className="flex flex-col gap-2">
                        <h1 className="text-[rgba(0,0,0,0.7)] font-semibold text-center">{title}</h1>
                        <span className="text-sm text-[rgba(0,0,0,0.7)] font-semibold">{ingredients}</span>
